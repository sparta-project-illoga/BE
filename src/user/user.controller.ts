...
export class UserController {
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      '`sendmail`: 이메일 인증번호 전송, `verifycode`: 인증번호 검증. 이 파라미터가 없는 경우 일반 회원가입을 수행합니다.',
  })
  @ApiCreatedResponse({ description: '유저를 생성한다', type: RegisterDto })
  @ApiResponse({ status: 201, description: '회원가입에 성공하였습니다' })
  @HttpCode(201)
  async register(@Query('type') type: string, @Body() body: any) {
    // 이메일 인증번호 전송 로직 실행
    if (type === 'sendmail') {
  ...
  @ApiCreatedResponse({
    ...
    schema: {
      example: {
        access_token:
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Impza2ltNDY5NUBnbWFpbC5jb20iLCJzdWIiOjcsImlhdCI6MTcxMjczNjQyMiwiZXhwIjoxNzEyNzM2NzIyfQ.0DtlYn-38fcLj8A95XItC11jgOaWbdM7L1GbX5EYVso',
        refresh_token:
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Impza2ltNDY5NUBnbWFpbC5jb20iLCJzdWIiOjcsImlhdCI6MTcxMjczNjQyMiwiZXhwIjoxNzEzMzQxMjIyfQ.YiQd2CG_gd80AcDqAAzAnQweBXtbFGtC6ezifslKaIU',
      },
    },
  })
  async login(
  ...
  @ApiOperation({
    summary: '비밀번호 찾기',
    description: '유저의 이메일, 이름, 전화번호를 통해 비밀번호를 찾습니다.',
  })
  @ApiResponse({ status: 204, description: '비밀번호 찾기 요청 성공' })
  @ApiResponse({ status: 400, description: '잘못된 요청' })
  @HttpCode(204)
  async findPassword(@Body() findPwDto: FindPwDto) {
  ...
  @ApiOperation({
    summary: '비밀번호 변경',
    description:
      '유저의 현재 비밀번호와 새로운 비밀번호를 사용하여 비밀번호를 변경합니다.',
  })
  @ApiResponse({ status: 200, description: '비밀번호 변경에 성공하였습니다.' })
  @ApiResponse({ status: 400, description: '잘못된 요청' })
  async changePw(@UserInfo() user: User, @Body() changePwDto: ChangePwDto) {
